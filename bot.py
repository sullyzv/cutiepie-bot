...
alt_intents = discord.Intents.default()
alt_intents.message_content = True
alt_intents.members = True
alt_bot = commands.Bot(command_prefix='!', intents=alt_intents)
...
@alt_bot.event
async def on_ready():
    print(f'Sniper online: {alt_bot.user}')
...
async def steal(interaction: discord.Interaction, link: str):
    if not os.getenv("ALT_TOKEN"): return await interaction.response.send_message("ALT_TOKEN not set in Railway.", ephemeral=True)
    await interaction.response.send_message("Joining target server... please wait.", ephemeral=True)
    try:
        code = link.split("/")[-1].split(" ")[0]
        async with aiohttp.ClientSession() as session:
            headers = {"Authorization": f"Bot {os.getenv('ALT_TOKEN')}"}
            async with session.get(f"https://discord.com/api/v10/invites/{code}", headers=headers) as resp:
                if resp.status != 200: return await interaction.followup.send("Invalid invite link.", ephemeral=True)
                data = await resp.json()
                guild_id = data["guild"]["id"]
        target_guild = alt_bot.get_guild(int(guild_id))
        if not target_guild:
            await alt_bot.join_invite(code)
            await asyncio.sleep(5)
            target_guild = alt_bot.get_guild(int(guild_id))
        if not target_guild: return await interaction.followup.send("Failed to join.", ephemeral=True)
        stolen_count = 0
                            ...
                            if a.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.mp4')):
                                ...
                                if ch_media: 
                                    ...
                                    if stolen_count >= 5: 
                                        ...
                                        return await interaction.followup.send(f"Stole {stolen_count} images! Left the target server.", ephemeral=True)
            except: pass
        await target_guild.leave()
        await interaction.followup.send(f"Finished. Stole {stolen_count} images. Left target server.", ephemeral=True)
...
async def main():
    async with bot:
        async with alt_bot:
            ...
            await alt_bot.start(os.getenv("ALT_TOKEN"))
